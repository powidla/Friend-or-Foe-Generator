function tempmodel_community = createBaseTempmetmodel_community_everyoneGrows( ind_vec, filedr_metmodels, g_tol )
...
tempmodel_community.A = sparse( (2*num_ec + nMs*num_nec + nMs), tnr );  %x2 rows for EC constraints, x1 for each internal metabolism and a constraint that everything grows above tolerance
tempmodel_community.lb = sparse(tnr,1);
tempmodel_community.ub = sparse(tnr,1);
tempmodel_community.rhs = zeros((2*num_ec + nMs*num_nec + nMs),1);
tempmodel_community.sense = [repmat('>',num_ec,1);repmat('<',num_ec,1);repmat('=',nMs*num_nec,1);repmat('>',nMs,1)];
...
for ii = 1:nMs
    ...
    %extra rows to ensure growth:
    tempmodel_community.A(2*num_ec + nMs*num_nec + ii, cum_bmi_vec(ii)) = 1;
    %flux constraints:
...
%afterwards add to the tempmodel:
%each microbe should grow above a tolerance:
tempmodel_community.rhs( (2*num_ec + nMs*num_nec + 1):end ) = g_tol;
